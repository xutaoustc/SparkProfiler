...
import com.ctyun.sparkprofiler.common.{AggregateMetrics, AppContext}
...
class StageSkewAnalyzer extends  AppAnalyzer {
  ...
  def computePerStageEfficiencyStatistics(ac: AppContext, out: mutable.StringBuilder): Unit = {
    ...
    val totalRuntime = ac.stageMap.map(x => {
      if (x._2.metrics.map.isDefinedAt(AggregateMetrics.executorRuntime)) {
        x._2.metrics.map(AggregateMetrics.executorRuntime).value
      } else {
    ...
    val totalIOBytes   = ac.jobMap.values.map ( x => (  x.metrics.map(AggregateMetrics.inputBytesRead).value
                                                      + x.metrics.map(AggregateMetrics.outputBytesWritten).value
                                                      + x.metrics.map(AggregateMetrics.shuffleWriteBytesWritten).value
                                                      + x.metrics.map(AggregateMetrics.shuffleReadBytesRead).value)
                                              ).sum
    ...
    ac.stageMap.keySet
      ...
      .filter( x => ac.stageMap.get(x).get.metrics.map.isDefinedAt(AggregateMetrics.executorRuntime))
      .foreach(x => {
        ...
        val used      = sts.metrics.map(AggregateMetrics.executorRuntime).value
        val wasted    = available - used
        val usedPercent = (used * 100)/available.toFloat
        val wastedPercent = (wasted * 100)/available.toFloat
        val stageBytes = sts.metrics.map(AggregateMetrics.inputBytesRead).value
                       + sts.metrics.map(AggregateMetrics.outputBytesWritten).value
                       + sts.metrics.map(AggregateMetrics.shuffleWriteBytesWritten).value
                       + sts.metrics.map(AggregateMetrics.shuffleReadBytesRead).value
      ...
      //val maxTaskMemoryUtilization = (maxTaskMemory*100)/executorMemory
        ...
        val taskRuntimePercent = (sts.metrics.map(AggregateMetrics.executorRuntime).value * 100)/totalRuntime.toFloat
        val idealWallClock = sts.metrics.map(AggregateMetrics.executorRuntime).value/(maxExecutors * executorCores)

        out.println (f"${x}%8s   ${stagePercent}%5.2f   ${taskRuntimePercent}%5.2f   ${sts.taskExecutionTimes.length}%7s  " +
          f"${IOPercent}%5.1f  ${bytesToString(sts.metrics.map(AggregateMetrics.inputBytesRead).value)}%8s " +
          f" ${bytesToString(sts.metrics.map(AggregateMetrics.outputBytesWritten).value)}%8s  " +
          f"${bytesToString(sts.metrics.map(AggregateMetrics.shuffleReadBytesRead).value)}%8s " +
          f" ${bytesToString(sts.metrics.map(AggregateMetrics.shuffleWriteBytesWritten).value)}%8s    " +
  ...
  def checkForGCOrShuffleService(ac: AppContext, out: mutable.StringBuilder): Unit = {
    ...
    ac.stageMap.keySet.toBuffer.sortWith( _ < _ )
      ...
      val totalExecutorTime     = sts.metrics.map(AggregateMetrics.executorRuntime).value
      //shuffleWriteTime is in nanoSeconds
      val writeTimePercent:Float = (sts.metrics.map(AggregateMetrics.shuffleWriteTime).value.toFloat * 100)/totalExecutorTime/(1000*1000)
      val readFetchPercent:Float  = (sts.metrics.map(AggregateMetrics.shuffleReadFetchWaitTime).value.toFloat * 100)/ totalExecutorTime
      val gcPercent:Float        = (sts.metrics.map(AggregateMetrics.jvmGCTime).value.toFloat * 100) / totalExecutorTime

      val available = totalCores * ac.stageMap.get(x).get.duration.get
      val stagePercent:Float = (available.toFloat*100/totalMillis)
      val parallelismRatio:Float  = sts.metrics.count.toFloat/totalCores
      ...
      val totalInput = sts.metrics.map(AggregateMetrics.inputBytesRead).value + sts.metrics.map(AggregateMetrics.shuffleReadBytesRead).value
      val totalOutput = sts.metrics.map(AggregateMetrics.outputBytesWritten).value+ sts.metrics.map(AggregateMetrics.shuffleWriteBytesWritten).value
