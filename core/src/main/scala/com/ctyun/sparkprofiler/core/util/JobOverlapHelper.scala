package com.ctyun.sparkprofiler.core.util

import com.ctyun.sparkprofiler.core.common.AppContext
import com.ctyun.sparkprofiler.core.common.AppContext
import com.ctyun.sparkprofiler.core.timespan.JobTimeSpan
...
object JobOverlapHelper {
  ...
  def makeJobLists(ac: AppContext): List[List[JobTimeSpan]] = {
    ...
    val jobGroupListByExecID = ac.jobSQLExecId.groupBy(x => x._2).map(x => x._2.map(x => ac.jobMap(x._1)).toList).toList
